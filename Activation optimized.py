...
activation_key_pressed = threading.Event()
...
def wait_for_activation():
    print("Press 's' to start eye detection...")

    while not activation_key_pressed.is_set():
        if cv2.waitKey(1) & 0xFF == ord('s'):
            print("Activation key pressed. Starting eye detection...")
            activation_key_pressed.set()

activation_thread = threading.Thread(target=wait_for_activation)
capture_thread = threading.Thread(target=capture_and_process)

activation_thread.start()
activation_thread.join()

if activation_key_pressed.is_set():
